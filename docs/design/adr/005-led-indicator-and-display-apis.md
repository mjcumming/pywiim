...
- **Control**: One path — `LED_SWITCH_SET`. For **WiiM** we set capability from **device class** (`is_wiim_device` static detection) — we **do not** probe with `LED_SWITCH_SET` at connect (that would turn the LED off). For **Arylic** we set capability True by vendor and **try-and-ignore** on write: call `LED_SWITCH_SET` and do not raise on failure.
...
- **Capability detection**: Arylic → set `supports_led_switch` True from vendor so we offer the control; client `set_led_switch` catches and ignores errors for Arylic. WiiM → set `supports_led_switch` True from static **WiiM device class** (same merge as other WiiM defaults); **no** connect-time `LED_SWITCH_SET` probe. OEM LinkPlay units that are not WiiM-class must not be marked supported without evidence — integrations can extend via profile/vendor if needed.
...
- Arylic devices get LED Indicator capability from vendor (no HTTP at connect); failures on write are ignored to avoid maintainability cost.
- WiiM devices get LED Indicator capability from **device class** at static merge time; **no** `LED_SWITCH_SET` call during capability detection (avoids turning the status LED off at connect).
